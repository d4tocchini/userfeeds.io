function buildTable(tableSelector,dataTableOptions){var table=$(tableSelector).DataTable(dataTableOptions);MOBILE.toggleMobileText(table.table().header());return table;}
function initResponsiveTable(tableSelector,desktopDataTableOptions,mobileColOrder){var $table=$(tableSelector);if(!$table.length){return;} 
desktopDataTableOptions.destroy=true; var mobileOptions={"scrollX":true,"scrollCollapse":true,"fixedColumns":{"leftColumns":2,"heightMatch":"auto"},};if(mobileColOrder!==undefined){mobileOptions.colReorder={"order":mobileColOrder};}
var mobileDataTableOptions=$.extend(mobileOptions,desktopDataTableOptions);var table;if(MOBILE.isMobile()){table=buildTable(tableSelector,mobileDataTableOptions);}else{table=$(tableSelector).dataTable(desktopDataTableOptions);}
function mobileToDesktopCallback(){ table.colReorder&&table.colReorder.reset(); desktopDataTableOptions.scrollX=true;table=buildTable(tableSelector,desktopDataTableOptions);rerenderCurrency();}
function desktopToMobileCallback(){table=buildTable(tableSelector,mobileDataTableOptions);rerenderCurrency();}
MOBILE.onBrowserResize(mobileToDesktopCallback,desktopToMobileCallback);}
function constrainName(data,type){if(!MOBILE.isMobile()||type!=='display'){return data;}
const MAX_CHARS=14;var $div=$('<div>');$div.html(data);var $nameContainer=$div.find('.currency-name-container'),name=$nameContainer.html();name=name.length>MAX_CHARS?name.substr(0,MAX_CHARS-2)+'...':name;$nameContainer.html(name);return $div.html();}
$(document).ready(function(){var currencyTableOptions={"bFilter":false,"bInfo":false,"bLengthChange ":false,"bPaginate":false,"bStateSave":false,"aoColumnDefs":[{"bSortable":false,"aTargets":[0]},{"sType":"string","aTargets":[1],"render":constrainName},{"sType":"currency","aTargets":[2]},{"sType":"price","aTargets":[3]},{"sType":"volume","aTargets":[4]},{"sType":"totalsupply","aTargets":[5]},{"sType":"percent","aTargets":[6]},{"bSortable":false,"aTargets":[7]},{"asSorting":["desc","asc"],"aTargets":[2,3,4,5,6]},]};initResponsiveTable('#currencies',currencyTableOptions,
[0,1,3,6,2,5,4,7]);var tables=$('#currencies-all, #assets-all').dataTable({"bFilter":true,"bInfo":false,"bLengthChange ":false,"bPaginate":false,"bStateSave":false,"aoColumnDefs":[{"bSortable":false,"aTargets":[0]},{"sType":"string","aTargets":[1]},{"sType":"string","aTargets":[2]},{"sType":"currency","aTargets":[3]},{"sType":"price","aTargets":[4]},{"sType":"totalsupply","aTargets":[5]},{"sType":"volume","aTargets":[6]},{"sType":"percent","aTargets":[7]},{"sType":"percent","aTargets":[8]},{"sType":"percent","aTargets":[9]},{"asSorting":["desc","asc"],"aTargets":[3,4,5,6,7,8,9]},],"sDom":'t'});function toggleMobileForSummaryTable(){MOBILE.toggleMobileText($('#currencies-all thead, #assets-all thead'));rerenderCurrency();}
if(MOBILE.isMobile()){MOBILE.toggleMobileText($('#currencies-all thead, #assets-all thead'));}
MOBILE.onBrowserResize(toggleMobileForSummaryTable,toggleMobileForSummaryTable);var assetTableOptions={"bFilter":false,"bInfo":false,"bLengthChange ":false,"bPaginate":false,"bStateSave":false,"aoColumnDefs":[{"bSortable":false,"aTargets":[0]},{"sType":"string","aTargets":[1],"render":constrainName},{"sType":"string","aTargets":[2]},{"sType":"currency","aTargets":[3]},{"sType":"price","aTargets":[4]},{"sType":"volume","aTargets":[5]},{"sType":"totalsupply","aTargets":[6]},{"sType":"percent","aTargets":[7]},{"bSortable":false,"aTargets":[8]},{"asSorting":["desc","asc"],"aTargets":[3,4,5,6,7]},]};initResponsiveTable('#assets',assetTableOptions,
[0,1,4,7,3,6,5,8,2]);$('#markets-table').dataTable({"bFilter":false,"bInfo":false,"bLengthChange ":false,"bPaginate":false,"bStateSave":false,"aoColumnDefs":[{"bSortable":false,"aTargets":[0]},{"sType":"innerstring","aTargets":[1]},{"sType":"innerstring","aTargets":[2]},{"sType":"volume","aTargets":[3]},{"sType":"price","aTargets":[4]},{"sType":"percent","aTargets":[5]},{"sType":"string","aTargets":[6]},{"asSorting":["desc","asc"],"aTargets":[3,4,5]},]});var currenciesVolume=$('#currencies-volume').DataTable({"bFilter":false,"bInfo":false,"bLengthChange ":false,"bPaginate":false,"bStateSave":false,"aoColumnDefs":[{"bSortable":false,"aTargets":[0]},{"sType":"string","aTargets":[1]},{"sType":"string","aTargets":[2]},{"sType":"volume","aTargets":[3]},{"sType":"volume","aTargets":[4]},{"sType":"volume","aTargets":[5]},{"asSorting":["desc","asc"],"aTargets":[3,4,5]},],"order":[]}); currenciesVolume.on('order.dt search.dt',function(){currenciesVolume.column(0,{order:'applied'}).nodes().each(function(cell,i){cell.innerHTML=i+1;});}).draw();jQuery.extend(jQuery.fn.dataTableExt.oSort,{"totalsupply-pre":function(a){if($(a).data("supply")){a=$(a).data("supply");}
a=parseFloat(a)
if(isNaN(a)){a=-1;}
return a;},"totalsupply-asc":function(a,b){return a-b;},"totalsupply-desc":function(a,b){return b-a;}});jQuery.extend(jQuery.fn.dataTableExt.oSort,{"currency-pre":function(a){if(a==='-'){a=0;}else{a=a.replace(/[^\d\-\.]/g,"");a=a.replace(/^\./,"");}
a=parseFloat(a);if(!a){a=0;}
return a;},"currency-asc":function(a,b){return a-b;},"currency-desc":function(a,b){return b-a;}});jQuery.extend(jQuery.fn.dataTableExt.oSort,{"price-pre":function(a){var $div=$('<div>');$div.html(a);var priceContainer=$div.find('.price'),$container=priceContainer.length?priceContainer:$div,text=$container.text();if(text=="?"){return 0;}
text=text.replace(/[^\d\-\.]/g,"");text=text.replace(/^\./,"");s=text.indexOf('-')
if(s>0){e=parseInt(text.substring(s+1))
text=parseFloat(text)*1/Math.pow(10,e)}else{text=parseFloat(text)}
if(!text){text=0;}
return text;},"price-asc":function(a,b){return a-b;},"price-desc":function(a,b){return b-a;}});jQuery.extend(jQuery.fn.dataTableExt.oSort,{"volume-pre":function(a){var $div=$('<div>');$div.html(a);var $container,volumeContainer=$div.find('.volume'),$container=volumeContainer.length?volumeContainer:$div,usd=$container.data('usd'),text=usd===undefined?$container.text():usd;text=parseFloat(text)
if(isNaN(text)){text=-1;}
return text;},"volume-asc":function(a,b){return a-b;},"volume-desc":function(a,b){return b-a;}});jQuery.extend(jQuery.fn.dataTableExt.oSort,{"percent-pre":function(a){a=parseFloat(a.replace(/[^\d\-\.]/g,""));if(isNaN(a)){a=-0.0001;}
return a;},"percent-asc":function(a,b){return a-b;},"percent-desc":function(a,b){return b-a;}});jQuery.extend(jQuery.fn.dataTableExt.oSort,{"innerstring-pre":function(a){a=$(a).text();return a;},"innerstring-asc":function(a,b){return a-b;},"innerstring-desc":function(a,b){return b-a;}});$('#tooltip-low-volume').tooltip();});function convert_percent(elem,data_symbol){var amount=elem.data(data_symbol);elem.removeClass("positive_change negative_change");if(amount<0){elem.addClass("negative_change")}else{elem.addClass("positive_change")}
elem.html(amount+"%")}
...
return val;}
function format_fiat_short(val){if(val>=1||val==0){if(val>=100000){val=Math.round(val).toLocaleString();}else{val=val.toFixed(2);}}else{if(val<0.01){val=val.toExponential();}else{val=val.toFixed(2);}}
return val;}
...
if(selector_type!=='.price'){amount=abbreviateIfMobile(amount);}}
...
function toggle_native(){$("#currency-switch-button").html("Native"+" <span class=\"caret\"></span>").data('currency','native');$.each([$('.price'),$('.volume')],function(){selector_type=this.selector
$.each(this,function(key,value){amount=$(this).data("native");if(amount==null){amount="N/A";}
else if(amount!="?"){amount=parseFloat(amount)
if(selector_type==".price"){amount=format_fiat(amount);}else{amount=format_market_cap(amount);}
if(selector_type!=='.price'){amount=abbreviateIfMobile(amount);}}
$(this).html(amount);});});}
function toggle_platform(){$("#currency-switch-button").html("Platform"+" <span class=\"caret\"></span>").data('currency','platform');$.each([$('.market-cap'),$('.price'),$('.volume')],function(){selector_type=this.selector
...
function tooltip_format_percentage(){val=this.y.toFixed(2)
return'<span style="color:'+this.color+'">\u25CF</span> '+this.series.name+': <b>'+val+'%</b><br/>'}
function label_format_market_cap(){val=format_market_cap(this.value)
return'$'+val;}
function label_format_crypto(){if(is_mobile()){val=format_crypto_short(this.value)}else{val=format_crypto(this.value)+' BTC'}
return val;}
function label_format_fiat(){if(is_mobile()){val=format_fiat_short(this.value)}else{val=format_fiat(this.value)}
return'$'+val;}
function HighChartsGraph(graphId,loadingId,noDataId)
{this.graphId=graphId;this.loadingId=loadingId;this.noDataId=noDataId;}
...
{var chart=$("#"+this.graphId).highcharts();return chart!==undefined;}
...
HighChartsGraph.prototype.bindLogScaleToggle=function(linearToggleElem,logarithmicToggleElem){var that=this;var chart=$('#'+that.graphId).highcharts();linearToggleElem.click(function(){chart.showLoading("Updating...");chart.update({yAxis:[{type:'linear'},{type:'linear',floor:0},{type:'linear',floor:0},{type:'linear'}]});dataLayer.push({'event':'customEvent','eventCategory':'Highcharts','eventAction':'Linear Toggled','eventLabel':that.uid});logarithmicToggleElem.removeClass("active");linearToggleElem.addClass("active");chart.hideLoading();});logarithmicToggleElem.click(function(){var marketCapEnabled=chart.series[0].visible&&chart.series[0].dataMax>0
chart.showLoading("Updating...");chart.update({yAxis:[{type:'logarithmic'},{type:'logarithmic',floor:'null',gridLineWidth:marketCapEnabled?0:1},{type:'logarithmic',floor:'null',gridLineWidth:marketCapEnabled?0:1},{type:'logarithmic'}]});dataLayer.push({'event':'customEvent','eventCategory':'Highcharts','eventAction':'Logarithmic Toggled','eventLabel':that.uid});linearToggleElem.removeClass("active");logarithmicToggleElem.addClass("active");chart.hideLoading();});}
...
function CurrencyDetailGraph(graphId,loadingId,noDataId){HighChartsGraph.call(this,graphId,loadingId,noDataId);this.slug=$("#metadata").data("slug");this.chartName=is_altcoin(this.slug)?"altcoin":"bitcoin";this.uid=$("#metadata").data("slug");}
CurrencyDetailGraph.prototype=new HighChartsGraph;CurrencyDetailGraph.constructor=CurrencyDetailGraph;CurrencyDetailGraph.prototype.finishUpdateCharts=function(seriesData){var that=this;var chart=$('#'+that.graphId).highcharts();chart.series[0].setData(seriesData["market_cap_by_available_supply"]);chart.series[1].setData(seriesData["price_usd"]);chart.series[2].setData(seriesData["price_btc"]);chart.series[3].setData(seriesData["volume_usd"]);chart.hideLoading();}
CurrencyDetailGraph.prototype.fetchAndLoad=function(callback,start,end){var that=this;var slug=$("#metadata").data("slug");var apiDomain=$("#metadata").data("apidomain");timeParams=""
if(start!==undefined&&end!==undefined){timeParams=start+"/"+end+"/";}
$.ajax({url:apiDomain+"/currencies/"+slug+"/"+timeParams,type:"GET",dataType:"json",error:function(){that.hideLoading();that.showNoData();},success:function(data){callback.call(that,data);}});}
CurrencyDetailGraph.prototype.initCharts=function(seriesData){var that=this;Highcharts.setOptions({lang:{numericSymbols:['k','M','B']}});var titleName=$("#metadata").data("name"); $('#'+that.graphId).highcharts('StockChart',{chart:{type:'line',zoomType:'x',height:is_mobile()?520:620,ignoreHiddenSeries:true},responsive:{rules:[{condition:{maxWidth:500},chartOptions:{chart:{zoomType:'none'},credits:{enabled:false},scrollbar:{enabled:false}}}]},tooltip:{shared:true,hideDelay:50,xDateFormat:'%A, %b %d %Y, %H:%M:%S UTC'},legend:{enabled:true,align:'center',backgroundColor:'#FFFFFF',borderColor:'black',borderWidth:0,layout:'horizontal',verticalAlign:'bottom',y:0,shadow:false,floating:false},navigator:{adaptToUpdatedData:false},scrollbar:{liveRedraw:false},title:{text:titleName+' Charts',align:"left",style:{fontSize:"24px"}},subtitle:{text:''},credits:{href:"https://coinmarketcap.com",text:"coinmarketcap.com",},rangeSelector:{allButtonsEnabled:true,buttons:[{type:'day',count:1,text:'1d'},{type:'week',count:1,text:'7d'},{type:'month',count:1,text:'1m'},{type:'month',count:3,text:'3m'},{type:'year',count:1,text:'1y'},{type:'ytd',count:1,text:'YTD'},{type:'all',text:'ALL'}],selected:6,inputEnabled:true, enabled:true},xAxis:[{events:{afterSetExtremes:function(e){that.afterSetExtremes(e)}},minRange:24*3600*1000
}],yAxis:[{labels:{formatter:function(){return'$'+this.axis.defaultLabelFormatter.call(this);},align:'right',style:{color:'#7cb5ec'}},title:{text:'Market Cap',style:{color:'#7cb5ec','font-weight':'bold'}},showEmpty:false,height:'80%',opposite:false,floor:0,},{labels:{formatter:label_format_fiat,style:{color:'#009933',},align:"left",x:15},title:{text:'Price (USD)',style:{color:'#009933','font-weight':'bold'}},showEmpty:false,height:'80%',opposite:true,floor:0},{labels:{formatter:label_format_crypto,style:{color:'#f7931a',},align:"left",x:15},title:{text:'Price (BTC)',style:{color:'#f7931a','font-weight':'bold'}},showEmpty:false,height:'80%',opposite:true,floor:0},{labels:{align:'right',style:{color:'#777',}},title:{text:'24h Vol',style:{color:'#777','font-weight':'bold'}},showEmpty:false,top:'80%',height:'20%',offset:2,lineWidth:1,opposite:false}],series:[{name:'Market Cap',color:'#7cb5ec',tooltip:{pointFormatter:tooltip_format_market_cap},data:seriesData["market_cap_by_available_supply"],visible:series_is_visible(this.chartName,0,true),dataGrouping:{enabled:false}},{name:'Price (USD)',yAxis:1,color:'#009933',tooltip:{pointFormatter:tooltip_format_fiat},data:seriesData["price_usd"],visible:series_is_visible(this.chartName,1,(!is_altcoin(this.slug)||!is_mobile())),dataGrouping:{enabled:false}},{name:'Price (BTC)',color:'#f7931a',yAxis:2,tooltip:{pointFormatter:tooltip_format_crypto},data:seriesData["price_btc"],visible:series_is_visible(this.chartName,2,is_altcoin(this.slug)),dataGrouping:{enabled:false}},{type:'column',name:'24h Vol',color:'#777',yAxis:3,tooltip:{pointFormatter:tooltip_format_market_cap},data:seriesData["volume_usd"],visible:series_is_visible(this.chartName,3,true),dataGrouping:{approximation:"average",enabled:false}}],plotOptions:{series:{events:{legendItemClick:function(event){var index=event.target.index
save_preferences(that.chartName,index,this.chart);}}}},});that.bindLogScaleToggle($('#js-linear-mode'),$('#js-logarithmic-mode'))
that.hideLoading();}
$(document).ready(function(){function loadWidgetJS()
{var head=document.getElementsByTagName('head')[0];var script=document.createElement('script');script.type='text/javascript';script.src='https://files.coinmarketcap.com/static/widget/currency.js';head.appendChild(script);}
$(function(){var hash=window.location.hash;if(!hash){hash="#charts"}else if(hash!="#charts"&&hash!="#social"&&hash!="#tools"){hash="#markets"}
var currencyDetailGraph=new CurrencyDetailGraph("highcharts-graph","highcharts-loading","highcharts-nodata")
if(hash=="#charts"&&!currencyDetailGraph.chartsLoaded()){currencyDetailGraph.init();}
if(hash=="#tools"){loadWidgetJS();}
$('ul.nav a[href="'+hash+'"]').tab('show');$('.nav-tabs a').click(function(e){$(this).tab('show');var scrollmem=$('body').scrollTop();window.location.hash=this.hash;$('html,body').scrollTop(scrollmem);if(this.hash=="#charts"&&!currencyDetailGraph.chartsLoaded()){currencyDetailGraph.init();}
if(this.hash=="#tools"){loadWidgetJS();}});});});